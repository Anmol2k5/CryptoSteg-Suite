...
<body class="min-h-screen bg-[#020203] text-gray-300 font-mono p-4 md:p-8 selection:bg-cyan-900 selection:text-cyan-100">
    ...
    <div class="max-w-7xl mx-auto space-y-8">
        ...
        <!-- LSB STEGANOGRAPHY STUDIO -->
        <div class="bg-[#0a0a0f] border border-cyan-800 rounded-sm flex flex-col overflow-hidden relative shadow-2xl">
            <div class="bg-[#111] border-b border-[#222] px-4 py-2.5 flex items-center justify-between select-none">
                <div class="flex items-center gap-2 text-cyan-400 text-xs font-mono tracking-wider uppercase">
                    <i data-lucide="key" class="w-4 h-4 text-cyan-400"></i>
                    LSB Steganography Studio (Encode & Decode Secret Messages)
                </div>
                <div class="flex gap-1.5">
                    <div class="w-2.5 h-2.5 rounded-full bg-cyan-900/50"></div>
                    <div class="w-2.5 h-2.5 rounded-full bg-cyan-900/50"></div>
            ...
            <div class="p-8 grid grid-cols-1 lg:grid-cols-2 gap-8">
                <!-- ENCODE CARD -->
                <div class="bg-[#050508] border border-[#222] p-6 rounded-sm flex flex-col justify-between">
                    <div>
                        <h3 class="text-sm uppercase text-gray-200 font-bold mb-4 flex items-center gap-2 border-b border-[#222] pb-2 text-cyan-400">
                            <i data-lucide="lock" class="w-4 h-4"></i> Hide Secret Data (Encode)
                        </h3>
                        <form action="/stego/encode" method="post" enctype="multipart/form-data" class="space-y-4">
                            <div class="flex flex-col gap-2">
                                <span class="text-gray-500 text-xs uppercase tracking-wider">Carrier Image Source</span>
                                <label class="group relative flex items-center justify-center w-full h-12 px-4 transition-all border border-dashed border-gray-700 rounded-sm bg-[#0a0a0f] hover:bg-[#0f0f15] hover:border-cyan-500/50 cursor-pointer">
                                    <div class="flex items-center gap-2 text-gray-500 group-hover:text-cyan-400 transition-colors">
                                        <i data-lucide="upload-cloud" class="w-4 h-4"></i>
                                        <span id="stego-enc-name" class="text-xs tracking-wider">Select_Carrier_PNG</span>
                                    </div>
                                    <input type="file" name="image" accept="image/*" required class="hidden" onchange="updateFileName(this, 'stego-enc-name')" />
                                </label>
                            </div>
                            <div class="flex flex-col gap-2">
                                <span class="text-gray-500 text-xs uppercase tracking-wider">Secret Message Payload</span>
                                <textarea name="message" rows="3" required placeholder="Enter confidential message to hide inside image..." class="w-full bg-[#0a0a0f] border border-gray-800 text-cyan-300 px-4 py-2.5 text-xs focus:outline-none focus:border-cyan-500 focus:ring-1 focus:ring-cyan-500/50 transition-all font-mono resize-none"></textarea>
                            </div>
                            <button type="submit" class="w-full border px-6 py-3 uppercase tracking-widest text-xs font-bold transition-all duration-300 flex items-center justify-center gap-2 bg-cyan-950/30 text-cyan-400 border-cyan-800 hover:bg-cyan-900/50 hover:border-cyan-500 hover:shadow-[0_0_15px_rgba(6,182,212,0.2)]">
                                <i data-lucide="file-key" class="w-4 h-4"></i> [ ENCODE_PAYLOAD ]
                            </button>
                        </form>
                    </div>
                    {% if stego_encoded_img %}
                    <div class="mt-6 border-t border-[#222] pt-4 flex flex-col gap-3">
                        <div class="inline-flex items-center gap-2 bg-emerald-950/40 border border-emerald-900/50 text-emerald-400 text-xs px-3 py-2 rounded-sm font-bold uppercase tracking-wider">
                            <i data-lucide="check-circle-2" class="w-4 h-4"></i> Successfully Encoded Message!
                        </div>
                        <a href="{{ stego_encoded_img }}" download="stego_secret.png" class="inline-flex items-center justify-center gap-2 bg-cyan-500 text-black px-4 py-2.5 rounded-sm text-xs font-bold hover:bg-cyan-400 transition-colors uppercase tracking-widest shadow-lg shadow-cyan-500/20">
                            <i data-lucide="download" class="w-4 h-4"></i> Download Stego-Image (PNG)
                        </a>
                    </div>
                    {% endif %}
                </div>
                ...
                <div class="bg-[#050508] border border-[#222] p-6 rounded-sm flex flex-col justify-between">
                    <div>
                        <h3 class="text-sm uppercase text-gray-200 font-bold mb-4 flex items-center gap-2 border-b border-[#222] pb-2 text-purple-400">
                            <i data-lucide="unlock" class="w-4 h-4"></i> Extract Secret Data (Decode)
                        </h3>
                        <form action="/stego/decode" method="post" enctype="multipart/form-data" class="space-y-4">
                            <div class="flex flex-col gap-2">
                                <span class="text-gray-500 text-xs uppercase tracking-wider">Stego-Image Source</span>
                                <label class="group relative flex items-center justify-center w-full h-12 px-4 transition-all border border-dashed border-gray-700 rounded-sm bg-[#0a0a0f] hover:bg-[#0f0f15] hover:border-purple-500/50 cursor-pointer">
                                    <div class="flex items-center gap-2 text-gray-500 group-hover:text-purple-400 transition-colors">
                                        <i data-lucide="upload-cloud" class="w-4 h-4"></i>
                                        <span id="stego-dec-name" class="text-xs tracking-wider">Select_Stego_PNG</span>
                                    </div>
                                    <input type="file" name="image" accept="image/*" required class="hidden" onchange="updateFileName(this, 'stego-dec-name')" />
                                </label>
                            </div>
                            <button type="submit" class="w-full border px-6 py-3 uppercase tracking-widest text-xs font-bold transition-all duration-300 flex items-center justify-center gap-2 bg-purple-950/30 text-purple-400 border-purple-800 hover:bg-purple-900/50 hover:border-purple-500 hover:shadow-[0_0_15px_rgba(168,85,247,0.2)]">
                                <i data-lucide="scan-text" class="w-4 h-4"></i> [ DECODE_PAYLOAD ]
                            </button>
                        </form>
                    </div>
                    {% if stego_decoded_msg %}
                    <div class="mt-6 border border-purple-900/50 bg-purple-950/20 p-4 rounded-sm flex flex-col gap-2">
                        <span class="text-xs uppercase text-purple-400 font-bold tracking-wider flex items-center gap-2 border-b border-purple-900/50 pb-2">
                            <i data-lucide="terminal" class="w-4 h-4"></i> Extracted Secret Payload:
                        </span>
                        <pre class="text-xs text-gray-100 font-mono whitespace-pre-wrap mt-1 overflow-x-auto p-2 bg-[#020203] rounded border border-[#222]">{{ stego_decoded_msg }}</pre>
                    </div>
                    {% elif stego_decode_error %}
                    <div class="mt-6 border border-red-900/50 bg-red-950/20 p-4 rounded-sm flex flex-col gap-2">
                        <span class="text-xs uppercase text-red-400 font-bold tracking-wider flex items-center gap-2 border-b border-red-900/50 pb-2">
                            <i data-lucide="alert-triangle" class="w-4 h-4"></i> Extraction Failed:
                        </span>
                        <p class="text-xs text-red-300 font-mono mt-1">{{ stego_decode_error }}</p>
                    </div>
                    {% endif %}
                </div>
            </div>
...
def stego_encode_route():
    ...
    from .stego_lsb import encode_lsb
    ...
    try:
        encode_lsb(in_path, out_path, message)
        with open(out_path, 'rb') as f:
            encoded_bytes = f.read()
        b64_img = base64.b64encode(encoded_bytes).decode('utf-8')
        data_url = f"data:image/png;base64,{b64_img}"
        return render_template_string(DASHBOARD_HTML, stego_encoded_img=data_url, batch_results=_load_batch_results())
    except Exception as e:
        return render_template_string(DASHBOARD_HTML, stego_decode_error=f"Encoding error: {e}", batch_results=_load_batch_results())
    finally:
        if os.path.exists(in_path):
...
def stego_decode_route():
    ...
    file = request.files['image']
    from .stego_lsb import decode_lsb
    ...
    try:
        secret_msg = decode_lsb(in_path)
        return render_template_string(DASHBOARD_HTML, stego_decoded_msg=secret_msg, batch_results=_load_batch_results())
    except Exception as e:
        return render_template_string(DASHBOARD_HTML, stego_decode_error=str(e), batch_results=_load_batch_results())
    finally:
        if os.path.exists(in_path):
            try: os.unlink(in_path)
            except: pass
